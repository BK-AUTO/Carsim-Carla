...
class FrontTireModel:
    """
    Front Tire Model based on Simulink diagram
    ...
    def __init__(self, vehicle_mass, wheelbase_length, cg_height, 
        ...
        """
        Initialize front tire model parameters
        ...
        Args:
            ...
            n_hs (float): Efficiency factor (default 1.0)
        """
        self.m = vehicle_mass
        ...
        # Internal states
        self.front_tire_angular_velocity = 0.0  # rad/s
        
    def calculate_front_vertical_load(self, vehicle_acceleration):
        """
        Calculate front tire vertical load (F_zf) based on Simulink diagram: F_zf_block
        Formula: Fzf = (g*lr + h*a) * m/l
        
        Args:
            vehicle_acceleration (float): Vehicle acceleration in m/s²
            
        Returns:
            float: Front tire vertical load in N
        """
        # From the Simulink block: (g*lr + h*acceleration) * m/l
        # Assuming lr (rear wheelbase distance) = l/2 for simplification
        # More accurate would be to pass lr as a parameter
        lr = 1.4455  # Simplified assumption
        
        fzf = (self.g * lr - self.h * vehicle_acceleration) * self.m / self.l
        return fzf
    
    def calculate_front_motor_angular_velocity(self, front_tire_angular_velocity):
        """
        Calculate front motor angular velocity from tire angular velocity
        Formula: w_motor = w_tire * i_tlc * i_hs
        
        Args:
            front_tire_angular_velocity (float): Front tire angular velocity in rad/s
            
        Returns:
            float: Front motor angular velocity in rad/s
        """
        w_motor = front_tire_angular_velocity * self.i_tlc * self.i_hs
        return w_motor
    
    def calculate_tire_slip(self, front_tire_angular_velocity, vehicle_velocity_ms):
        """
        ...
        if abs(vehicle_velocity_ms) > 0.01:  # Avoid division by zero
            slip_ratio1 = (self.r_wh * front_tire_angular_velocity - vehicle_velocity_ms) / vehicle_velocity_ms
        else:
            slip_ratio1 = 0.0
        
        # Calculate slip ratio 2: (r_wh * w - v) / (r_wh * w)
        wheel_velocity = self.r_wh * front_tire_angular_velocity
    ...
    def calculate_traction_force(self, slip_ratio, front_vertical_load):
        """
        Calculate front tire traction force based on slip ratio and vertical load
        ...
        Args:
            ...
            front_vertical_load (float): Front vertical load in N
            
        Returns:
            float: Front tire traction force in N
        """
        ...
        traction_force = traction_coefficient * 0.9 * front_vertical_load
        
        # Apply saturation to prevent unrealistic forces
        max_traction = front_vertical_load * 1.0  # Maximum mu ~ 1.0
        traction_force = np.clip(traction_force, -max_traction, max_traction)
        
        return traction_force
    
    def calculate_tire_angular_acceleration(self, front_motor_torque, front_mechanical_braking_torque,
                                           front_traction_force, front_vertical_load):
        """
        Calculate front tire angular acceleration based on torque balance
        From Simulink Front Tire Model block:
        ...
        Args:
            front_motor_torque (float): Front motor torque in Nm
            front_mechanical_braking_torque (float): Mechanical braking torque in Nm
            front_traction_force (float): Front tire traction force in N
            front_vertical_load (float): Front tire vertical load in N
            
        Returns:
            float: Front tire angular acceleration in rad/s²
        """
        ...
        motor_torque_contribution = front_motor_torque * self.i_tlc * self.n_tlc * self.i_hs * self.n_hs
        
        # Traction force torque (opposing motion)
        traction_torque = front_traction_force * self.r_wh
        ...
        vertical_torque = front_vertical_load * self.r_wh * self.id_f  # This might need clarification
        
        # Total torque balance (note the signs from Simulink)
        net_torque = motor_torque_contribution - front_mechanical_braking_torque - traction_torque - vertical_torque
    ...
    def update_tire_angular_velocity(self, angular_acceleration, dt):
        ...
        Returns:
            float: Updated front tire angular velocity in rad/s
        """
        self.front_tire_angular_velocity += angular_acceleration * dt
        
        # Prevent negative angular velocity (wheel can't spin backwards in this simplified model)
        if self.front_tire_angular_velocity < 0:
            self.front_tire_angular_velocity = 0
        
        return self.front_tire_angular_velocity
    
    def get_outputs(self, vehicle_acceleration, vehicle_velocity_ms, 
                   front_motor_torque, front_mechanical_braking_torque, 
                   current_time, dt=0.1):
        """
        Main function to calculate all front tire outputs
        ...
        Returns:
            dict: Dictionary containing all front tire outputs
        """
        # 1. Calculate front vertical load
        front_vertical_load = self.calculate_front_vertical_load(vehicle_acceleration)
        
        # 2. Calculate tire slip
        tire_slip = self.calculate_tire_slip(self.front_tire_angular_velocity, vehicle_velocity_ms)
        
        # 3. Calculate traction force
        front_traction_force = self.calculate_traction_force(tire_slip, front_vertical_load)
        ...
        front_tire_angular_velocity = self.update_tire_angular_velocity(tire_angular_acceleration, dt)
        
        # 6. Calculate front motor angular velocity
        front_motor_angular_velocity = self.calculate_front_motor_angular_velocity(front_tire_angular_velocity)
        
        return {
            'front_vertical_load': front_vertical_load,  # N
            'front_tire_slip': tire_slip,  # dimensionless
            'front_traction_force': front_traction_force,  # N
            'front_tire_angular_velocity': front_tire_angular_velocity,  # rad/s
            'front_tire_angular_acceleration': tire_angular_acceleration,  # rad/s²
            'front_motor_angular_velocity': front_motor_angular_velocity,  # rad/s
            'time': current_time
        }
    
    def reset_states(self):
        """Reset all internal states"""
        self.front_tire_angular_velocity = 0.0


# Example usage and testing
if __name__ == "__main__":
    # Create front tire model instance with typical values
    ...
    print("Testing Front Tire Model...")
    print("=" * 80)
    ...
    print("-" * 80)
    print(f"{'Time':<6} {'Fzf':<10} {'Slip':<10} {'Fxf':<10} {'w_tire':<12} {'w_motor':<12}")
    ...
    for t in time_steps:
        # Get outputs
        outputs = front_tire.get_outputs(
        ...
        # Print every second
        if abs(t % 1.0) < dt/2:
            print(f"{t:<6.1f} {outputs['front_vertical_load']:<10.1f} "
                  f"{outputs['front_tire_slip']:<10.4f} {outputs['front_traction_force']:<10.1f} "
                  f"{outputs['front_tire_angular_velocity']:<12.2f} "
                  f"{outputs['front_motor_angular_velocity']:<12.2f}")
    
    print("\n" + "=" * 80)
    print("Test completed!")
    
    # Print final state
    print(f"\nFinal front tire angular velocity: {front_tire.front_tire_angular_velocity:.2f} rad/s")
